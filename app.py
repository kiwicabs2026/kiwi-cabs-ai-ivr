...
def make_twiml_speech_response(text):
    ...
    gather = Gather(input="speech", action="/process_speech", method="POST", timeout=5, bargeIn=True)
...
def voice():
    response = VoiceResponse()
    gather = Gather(input="speech", action="/menu", method="POST", timeout=5, bargeIn=True)
    gather.say(
        "<speak>Kia ora, and welcome to Kiwi Cabs."
        "<break time='400ms'/>"
        "I am A dot I assistant, here to help you book your taxi."
        "<break time='500ms'/>"
...
def menu():
    ...
    if "1" in speech_result or "option one" in speech_result:
    ...
    if step == "collect_details":
        try:
            ai_response = openai.ChatCompletion.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "You are a helpful AI assistant for Kiwi Cabs in Wellington, New Zealand. IMPORTANT: We only operate within Wellington region. If pickup or dropoff is outside Wellington, reply with {\"error\": \"outside_wellington\", \"message\": \"Sorry, we only operate in Wellington region.\"}. Otherwise, return JSON: {\"name\", \"pickup\", \"dropoff\", \"time\"}. Caller may say option one, two, three."},
        ...
        if "yes" in speech.lower():
            ...
            try:
                ...
                print("POST to Render failed:", post_err)

            response = VoiceResponse()
            response.say("Thanks. Your taxi has been booked. Thank you for using Kiwi Cabs. Goodbye.", language="en-NZ")
            response.hangup()
            return str(response)
